...
int xilinx_xcvr_calc_cpll_config(struct xilinx_xcvr *xcvr,
	unsigned long refclk_khz, unsigned long lane_rate_khz,
...
int xilinx_xcvr_cpll_calc_lane_rate(struct xilinx_xcvr *xcvr,
	unsigned int ref_clk_khz, const struct xilinx_xcvr_cpll_config *conf,
	unsigned int out_div)
{
	if (conf->refclk_div == 0 || out_div == 0)
		return 0;

	return (ref_clk_khz * conf->fb_div_N1 * conf->fb_div_N2 * 2) /
...
int xilinx_xcvr_qpll_calc_lane_rate(struct xilinx_xcvr *xcvr,
	unsigned int ref_clk_khz, const struct xilinx_xcvr_qpll_config *conf,
	unsigned int out_div)
{
	if (conf->refclk_div == 0 || out_div == 0)
		return 0;

	return (ref_clk_khz * conf->fb_div) / (conf->refclk_div * out_div);
}
